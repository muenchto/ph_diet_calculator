...
export class AppComponent {
  ...
  sliders: SliderData[] = [...Array(this.numOfSlider).keys()].map((i) => ({
    name: 'Slider ' + i,
    value: this.localEqu,
    min: this.localMin,
    max: this.localMax,
  ...
  updateSliders(
    ...
    const sliderValDiff =
      sliderChange.value - this.sliders[adjustedSliderIdx].value;

    const isSliderAtLimit = (slider: SliderData) =>
      sliderValDiff > 0
    ...
    const numOfOtherSliders = this.sliders
      ...
      .filter((slider) => !isSliderAtLimit(slider)).length;
    ...
    this.actualGlobal = this.sliders
      .map((sl) => sl.value)
